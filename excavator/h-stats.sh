...
get_cards_hashes(){
        # hs is global
        hs=''
        for (( i=0; i < ${GPU_COUNT_TOTAL}; i++ )); do
                local MHS=`cat $LOG_NAME | grep "Device \#${i}-${i}" | tail -n 1 | awk '{ printf $6"\n" }' | sed -e 's/M.*//'`
...
get_total_hashes(){
        # khs is global
        local Total=`cat ${LOG_NAME} | grep -a "Algorithm" | grep -a "speed" | tail -n 1 | awk '{printf "%.f",$7*1000}'`
        echo $Total
}

get_log_time_diff(){
        local getLastLogTime=`tail -n 100 $LOG_NAME | grep -a "Algorithm" | grep -a "speed" | tail -n 1 | awk {'print $1,$2'} | sed 's/[][]//g'`
...
GPU_COUNT_TOTAL=`expr $GPU_COUNT_AMD + $GPU_COUNT_NVIDIA`
...
if [ "$diffTime" -lt "$maxDelay" ]; then
        ...
        local algo="x16r"                                       # algo

        # A/R shares by pool
        local ac=`cat $LOG_NAME | grep "Pool" | grep -c "accepted"`
        local rj=`cat $LOG_NAME | grep "Pool" | grep -c "rejected"`
